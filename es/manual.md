...
Empezar de nuevo / Guardar y salir / Salir sin guardar: Pulsando el siguiente botón tenemos tres opciones.  Borrar y comenzar de nuevo, salir a la pantalla principal del programa sin guardar, o guardar el trabajo.
...
#Modo de trabajo AB
El modo de trabajo AB permite a la aplicación mostrar referencias para el guiado durante el trabajo. Para usar este modo basta con marcar la primera pasada en la finca y la aplicación generará el resto de líneas que se deberán seguir. El siguiente gráfico muestra donde se deberían presionar los botones
...
El trazado de estas líneas en la pantalla va a permitir hacer pasadas alternas. En cada pasada hay que ajustarse a la línea correspondiente a la pasada que se quiere realizar. Es importante que la línea cruce al tractor justo por el medio.
##Actualización del programa
Los pasos a seguir para actualizar el programa “Agroguia” son los siguientes:
a) Una vez que la tablet está conectada a internet, con Agroguia arrancado, pulsar en la tecla de menú como aparece en la siguiente imagen:
...
Todo trabajo que hemos realizado se puede guardar, independientemente de si está acabado o no. Una de la utilidades que tiene Agroguia es, que si no hemos finalizado un trabajo y no lo vamos a terminar en ese instante, se puede guardar para luego, cuando volvamos a la parcela, tener almacenado lo que se ha tratado hasta ese momento. También podemos querer guardar un trabajo para poderlo ver en el ordenador sobre una foto de satélite, donde podemos extraer diferentes datos, como fecha de comienzo y fin del trabajo, si se midió el área, zonas en las que se cometió algún error, etc.
